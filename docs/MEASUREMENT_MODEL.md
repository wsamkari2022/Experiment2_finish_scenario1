...
Everything in this subsection is regenerated by **`npm run report:stability`**
(`tools/stability_distribution.cjs`). It runs the real scoring functions over randomly drawn
starting profiles, so the figures are a property of the instrument, not of one archetype. Quote them
from a fresh run rather than from memory: they move whenever the deck, the roles or the bump
magnitudes move.

**The number is a composite of two halves, averaged.**

```
order half    = 100 × (6 − pairs swapped) / 6          start versus end, 4 values = 6 pairs
movement half = 100 × (1 − min(1, churn / 56))         churn summed step by step
Stability     = (order + movement) / 2
```

**The scale is used end to end.** Across 1,200 runs (300 random starting profiles × 4 behaviors) the
lowest score observed was **0** and the highest **100**. No rescaling is needed or wanted, and the
churn ceiling of 56 is a *churn* value — it is not a cap on the score, and nothing in the model
prevents a participant reaching 100.

| Behavior | min | p25 | median | p75 | max | median reads as |
|---|---|---|---|---|---|---|
| Always the best-fitting option | 44 | 82 | 93 | 97 | 100 | Held steady |
| Always the second option | 25 | 60 | 74 | 86 | 100 | Mostly steady |
| Always the worst-fitting option | 0 | 17 | 25 | 35 | 51 | Changed substantially |
| Picks at random | 6 | 34 | 43 | 53 | 96 | Shifted a lot |

#### Three properties to state before reporting any Stability figure

**1. It is descriptive, not evaluative.** A high score means the participant's values did not move;
a low score means they did. Neither is a better outcome, and neither is a measure of decision
quality. The measure that asks whether choices matched values is VCI, and the two must never be
described as though a high number on one implies anything about the other.

**2. The order half is coarse, not continuous.** With four values there are six pairs, so it can
only take seven values: 0, 17, 33, 50, 67, 83, 100. Do not fit a model that assumes it is
continuous, and do not report a mean of the order half without saying how few levels it has.

**3. The movement half is censored from below.** It is exactly 0 for any participant whose churn
reaches 56, so participants past the ceiling are indistinguishable on that half. In the archetype
suite both Flip-flopper (churn 60.0) and Contrarian (churn 58.4) score 0 on movement and separate
only on the order half, 67 against 33. **Report the composite. Do not report the movement half alone
as an interval measure at the unstable end**, and if churn itself is analysed, say that it is the
uncensored quantity and the movement half is not.

#### A consistent participant can still score low, and that is correct

Over 2,000 simulated participants who chose their **best-fitting option in every scenario**:

| Band | Share |
|---|---|
| 85 or above, "Held steady" | 67.5% |
| 70 to 84, "Mostly steady" | 22.6% |
| 50 to 69, "Shifted a little" | 8.9% |
| Below 50 | 0.9% |

Median 92. About one in ten still lands below 70, and in **100%** of those cases the cause is the
order half: their four values changed rank order while they kept choosing what fitted them. That is
the measure working — Stability is about the values, not the choices — but it is the single most
likely thing to be misread, by a participant looking at the screen and by a reader looking at a
scatter plot. The results page therefore states in the caption that the number is about whether
values moved rather than whether the participant chose well, and points at the consistency card.

#### Visualizing it

- **Plot the composite**, and put the two halves beside it only when the point being made is about
  the halves. A reader shown the movement half alone will read the pile-up at 0 as a floor effect in
  the participants rather than in the instrument.
- **Show the distribution, not just a mean.** The four behavior rows above are close to uniform
  across the range, so a single average hides nearly everything.
- **Pair it with VCI on the same figure** when arguing about the CVR or APA, and label the axes with
  the questions rather than the names: "did your values move" against "did your choices fit your
  values". The names invite the reader to assume one is a better version of the other.
- **Do not colour low scores as bad.** The app deliberately avoids this and an analysis that
  reintroduces it is making a claim the measure does not support.
- If a figure needs the null reference, the churn null model is the honest one: p99 = 56.3 on the
  current deck, which is where the ceiling comes from. Gate S7 re-measures it on every run.
