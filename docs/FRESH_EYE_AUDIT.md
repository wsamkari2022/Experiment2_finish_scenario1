...
| A9 | The participant's own four value numbers are on screen in every scenario while they choose (the sidebar card, since 26 Sept the values panel), the same kind of number removed from the confirm-keep question | Medium, decision | Open (Fix 1): keep, or show only in development |
| A10 | Scenario 6 shows those four numbers although its four options ARE the four values (the meanings are hidden there for exactly that reason) | Medium, decision | Open (Fix 1): keep, or hide the panel in scenario 6 |
...
| D1 | Option numbers disagree with the option's own words | Critical | **Mostly resolved** 26 Sept: 7 of my 9 examples were already right under the 18 Sept value audit (I1); the last 2 fixed (Fix 5). The independent rater study (Fix 3, Step B) is still open |
